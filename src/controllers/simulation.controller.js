const { calculateGame, runMonteCarlo } = require('../services/gameEngine');
const { getCachedParams, DEFAULT_PARAMS } = require('../models/params.model');

// POST /api/simulations/calculate
exports.calculate = (req, res) => {
  try {
    const result = calculateGame(req.body, getCachedParams());
    res.json(result);
  } catch (err) {
...
exports.monteCarlo = (req, res) => {
  try {
    ...
    const result = runMonteCarlo(count, getCachedParams());
    res.json(result);
  } catch (err) {
...
// GET /api/simulations/rules
exports.getRules = (req, res) => {
  const p = getCachedParams();
  res.json({
    atkTable: p.atkTable,
    opTable:  p.opTable,
    fundTable: p.fundTable,
    initial: p.initial,
    roles: [
